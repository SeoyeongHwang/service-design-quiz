...
- 저장 파일명: drafts/batches/development_operation.js

목표:
- 서비스·경험디자인 개발 및 운영 섹션용 신규 10문항을 만든다.
...
export const batchMeta = {
  ...
  notes: "서비스·경험디자인 개발 및 운영 섹션 누적 초안. 첫 추가 문항은 011-020.",
};
...
이번 프롬프트의 기본 생성 범위:
- section은 항상 "development_operation"이다.
- 첫 추가 문항은 development_operation-011부터 development_operation-020까지다.
- sourceType은 모두 "theory"다.
- 문제는 서비스·경험디자인 개발 및 운영 범위만 다룬다.

중요한 추가 규칙:
- 만약 이미 `drafts/batches/development_operation.js`에 더 뒤 번호 문항이 있다면, 기존 번호와 중복되지 않도록 다음 번호부터 이어서 생성하라.
- 항상 id 중복을 피하라.
