...
- 저장 파일명: drafts/batches/strategy.js

목표:
- 사용자 중심 전략수립 섹션용 신규 10문항을 만든다.
...
export const batchMeta = {
  ...
  notes: "사용자 중심 전략수립 섹션 누적 초안. 첫 추가 문항은 011-020.",
};
...
이번 프롬프트의 기본 생성 범위:
- section은 항상 "strategy"다.
- 첫 추가 문항은 strategy-011부터 strategy-020까지다.
- sourceType은 모두 "theory"다.
- 문제는 사용자 중심 전략수립 범위만 다룬다.

중요한 추가 규칙:
- 만약 이미 `drafts/batches/strategy.js`에 더 뒤 번호 문항이 있다면, 기존 번호와 중복되지 않도록 다음 번호부터 이어서 생성하라.
- 항상 id 중복을 피하라.
...
- 특정 한 소주제에만 몰리지 말고 전략수립 범위를 넓게 커버하라
