...
- 출력 결과는 `drafts/batches/strategy.js`에 바로 붙여넣어 저장할 수 있는 완전한 JavaScript 파일이어야 한다.
- 기존 `strategy.js` 내용을 함께 제공받았다면 기존 문항을 유지하고 신규 문항 30개를 뒤에 이어 붙인 완성 파일을 출력한다.
...
- QUESTION_COUNT = 30
- DEFAULT_START_ID = 11
- 기본 생성 범위는 `strategy-011`부터 `strategy-040`까지다.
...
목표:
- 사용자 중심 전략수립 섹션용 신규 QUESTION_COUNT개 문항을 만든다.
...
필드 규칙:
- id: `strategy-011`처럼 3자리 번호를 사용하고 중복을 피한다.
- section: 항상 "strategy"
...
- 쉬운 정의 확인 문제보다 비교, 사례 적용, 순서 판단, 전략 도구 구분, 의사결정 맥락 판단 문제를 우선한다.
...
- medium 난이도: 최소 40%
- low 난이도: 최대 30%
- 특정 한 소주제에만 몰리지 말고 사용자 중심 전략수립 범위를 넓게 커버하라.
...
- 출력된 내용을 그대로 `drafts/batches/strategy.js`에 붙여넣어도 `npm run validate:batch -- drafts/batches/strategy.js`를 통과해야 한다.
